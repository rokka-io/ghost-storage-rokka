  ...
  async save(image, noFace) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.rokka.sourceimages.create(this.org, image.originalname, stream, meta).then(res => {
        ...
        const link = 'https://' + this.org + '.rokka.io/' + this.defaultStack + '/' + rokkaImage.short_hash + '/' + encodeURIComponent(rokkaImage.name.replace(/\.[a-zA-Z]{3,4}$/,"").replace(".","_")) + '.' + rokkaImage.format
        debug('Uploaded:', link)
        resolve(link)
